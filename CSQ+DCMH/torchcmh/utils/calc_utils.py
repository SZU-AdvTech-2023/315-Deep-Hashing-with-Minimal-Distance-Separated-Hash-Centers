...
def calc_map_k(qB, rB, query_L, retrieval_L, k=None):
    ...
    for iter in range(num_query):
        ...
        gnd = q_L.mm(retrieval_L.transpose(0, 1)) > 0
        gnd = gnd.clone().detach().squeeze().type(torch.float32)
        tsum = torch.sum(gnd)
        if tsum == 0:
            continue
        ...
        ind.squeeze_()
        device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")#移动到gpu
        gnd = gnd.to(device)
        gnd = gnd[ind]
...
def calc_neighbor2(label1, label2):
    # calculate the similar matrix2
    s1 = label1.matmul(label2.transpose(0, 1)) > 0
    s1 = s1 + 0
    s2 = 1 - s1
    Sim = s1 - s2
    return Sim.float()
...
def calc_neighbor3(label1, label2, bit: int):
    # calculate the similar matrix with weight
    s1 = label1.matmul(label2.transpose(0, 1)) > 0
    s1 = s1 + 0
    ratio = torch.sum(s1) / torch.sum(1-s1)
    Sim = s1*(1+ratio) - ratio
    Sim = Sim * bit
    return Sim
