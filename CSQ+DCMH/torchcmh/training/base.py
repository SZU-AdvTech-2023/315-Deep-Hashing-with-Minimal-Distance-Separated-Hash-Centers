...
class TrainBase(object):
    def __init__(self, name, data_name, bit, batch_size, visdom=False, cuda=True):
        self.parameters = {}
        self.loss_store = []
        self.name = name
        self.bit = bit
    ...
    def valid(self, epoch):
        ...
        if mapt2i + mapi2t >= self.max_mapi2t + self.max_mapt2i:
            ...
            #self.img_model.module.save_dict(os.path.join(self.checkpoint_dir, str(self.bit) + '-' + self.img_model.module.module_name + '.pth'))
    ...
    def get_codes(img_model, txt_model, dataset, bit, batch_size, cuda=True):
        ...
        for data in tqdm(dataloader):
            ...
            #CSQ专用
            u1,u2 = img_model(img,None, None)
            img_hash = torch.tanh(u1)
