...
class OurLoss(NN.Module):
    ...
    def forward(self, u1, u2, y, ind, img, U, k=0):
        self.U[ind, :] = u2.data
        self.Y[ind, :] = y
        self.img = img
        return self.cos_pair(u1, y, ind, img, U, k)

    
    def cos_pair(self, u, y, ind, img, U, k):
        ...
        Mutual_loss =  torch.norm(img - torch.mm(u, U), p=2)
        
        loss = cos_loss + self.config['beta'] * pair_loss + self.config['lambda'] * Q_loss + 0.000001 * Mutual_loss
